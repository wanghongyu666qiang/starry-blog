...
├─ public/                        # 头像、图片等静态资源
├─ src/
│  ├─ app/                        # 页面（含专栏）、Metadata、RSS、Sitemap、Robots
...
| `/columns`、`/columns/algorithms` | 专栏聚合与“算法札记”学习路线 |
| `/about` | 关于页面 |
| `/resume` | 在线简历 |
| `/feed.xml` | RSS |
| `/sitemap.xml` | Sitemap |
...
专栏聚合页位于 `src/app/columns/page.tsx`，共享的专栏名称、定位、语言、文章标签和学习路线定义在 `src/lib/columns.ts`。

“算法札记”页面位于 `src/app/columns/algorithms/page.tsx`，路由为 `/columns/algorithms`。它面向 LeetCode 面试，默认使用 C++，并从公开文章中自动收录带有 `算法` 标签的文章。

算法文章仍按第 2.1 节的流程保存到 `src/content/posts/`。它们可以继续使用 `Learning` 分类，但必须在 `tags` 中加入 `算法`才会进入专栏。`published: false` 的未完成草稿不会显示。

增加新专栏时，至少需要：

1. 在 `src/lib/columns.ts` 增加定义；
2. 在 `src/app/columns/` 下增加对应页面；
3. 在 `src/app/sitemap.ts` 加入新路由；
4. 补充测试、Metadata，并检查桌面端和移动端。
...
| 专栏配置与学习路线 | `src/lib/columns.ts`、`src/app/columns/` |
...
- [ ] 首页、文章、项目、专栏、About、Resume 正常；
- [ ] `/columns` 与 `/columns/algorithms` 可访问，算法专栏空状态或文章列表正确；
- [ ] 桌面与移动导航正常；
- [ ] 主题切换和刷新持久化正常；
...
- `src/app/sitemap.ts` 生成 `/sitemap.xml`，包含静态页面、专栏与所有公开文章、项目；新增重要页面时需同步增加。
...
| `src/app/columns/` | 专栏聚合页、学习路线与专栏文章列表 |
...
| `src/lib/columns.ts` | 专栏定义、学习路线和文章归属规则 |
